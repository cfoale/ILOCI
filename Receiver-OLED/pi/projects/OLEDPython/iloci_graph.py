#ILOCIc Version 0.9 corresponding to ILOCIb Version 0.9  6/26/16 - cmf
#changed oofthreshold =  16, and scalefactor = 31.
#log now collects the ILOCIb float value scaled by scalefactor
...
def command2(bOn):   # function for sending a command back to data-recorder Pi upon button push(new command will be update command) - ICF-cmf
	...
	text = 'ILOCI Paused..'
	led.draw_text2(0,0,text,1)
	text2 = 'Press Pb1 to exit'
	led.draw_text2(0,16,text2,1)
	led.display()
	sleep(2)
	for i in range(0,100): #wait for either pb1 or pb2 for 10 seconds
		sleep(.1)
		try:
			nbytes = ser.inWaiting()
			if nbytes > 0:
			...
			if GPIO.input(Pb1) == False: # this is to do the exit 
				text2 = 'Pb1 pressed...          '
				led.draw_text2(0,16,text2,1)
				led.display()
				print('Send Command2')   # tells IFIb to unpause sending, and only continues when it receives another pause command
				ser.write('CMD2\n')		#an acknowledgement is always terminated with \n
				break
...
def CheckAction(): #checks the global variable action and displays in English what to do
	if action == 4:
		#put some text on the display as to what we are doing
		led.clear_display()
		#text = 'ILOCI'
		#led.draw_text2(0,0,text,1)
		text2 = 'LEFT RUDDER!'
		led.draw_text2(0,0,text2,2)
		led.display()
		sleep(2) #note the comm buffer is filling up with data from ILOCIb
...
width = 128
height = 32
############ show what script we are running ###################
...
#draw a sine curve to demonstrate the display
	...
	while True:
		...
		try:
			...
			if nbytes > 0:
				singleread = ser.readline()
				print(singleread)
				#strip off the leading digit to determine the action N,D,L, R
				classified = float(singleread)
				normalprob = classified - int(classified)
				action = int(classified - normalprob)
				#if the normal probability is 1.000, change the action to 0 and probability to 1
				if action == 1:
					action = 0
					normalprob = 1
					
				timems = millis()
				logfile.write(str(timems) + ' ' + str(normalprob) + ' ' + str(action) + '\n')
				print(str(timems) + ' ' + str(normalprob) + ' ' + str(action)) #our debug output
				
				 #scale the probability plot
				inum =int( scalefactor * normalprob)
				...
				#When plotting the history, y=0 is at the top, so store 31 - 
				...
				# This includes the sending acknowledgement
				bOn = (oof > oofthreshold)  # this is our Out of Family threshhold
				if oof > oofthreshold:
					GPIO.output(LED_out, True)	#we are Out of Family - turn the light on
					ledoncountdowntimer = 50; #for the time the data is on the display we will stay on before resetting, to attract attention to display
				...
				#see if the action requires immediate display of D, L, R
				CheckAction();
